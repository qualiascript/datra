...
\newcommand{\StaConfDaMon}{\mathsf{StaConfDaMon}}
...
\(\StaConfDaMon\) monoidal category, but instead of introducing it in
...
\(\StaConfDaMon\) can represent multiple atlas objects, by usual sum types
logic. Atlas morphisms are flexible, but morphisms in \(\StaConfDaMon\) are
more restrictive. Let us look at the case of two atlas objects represented
by \(\StaConfDaMon\) and a morphism between the two. The rules are as follows:
...
different category than \(\StaConfDaMon\), however, all these rules are
imposed in \(\StaConfDaMon\) itself.
...
\(\StaConfDaMon\) morphisms and puts them side by side. It can be seen as
...
which I believe \(\StaConfDaMon\) has enough flexibility to model in a
type-safe manner.
