...
\newcommand{\StaConfDaTravMon}{\mathsf{StaConfDaTravMon}}
...
\(\StaConfDaTravMon\) monoidal category, but instead of introducing it in
...
can recover a map from it by using the Charter functor, as detailed in the
preprint.
...
\(\StaConfDaTravMon\) can represent multiple atlas objects, by usual sum types
logic. Atlas morphisms are flexible, but morphisms in \(\StaConfDaTravMon\) are
more restrictive. Let us look at the case of two atlas objects represented
by \(\StaConfDaTravMon\) and a morphism between the two. The rules are as follows:
...
different category than \(\StaConfDaTravMon\), however, all these rules are
imposed in \(\StaConfDaTravMon\) itself.
...
\(\StaConfDaTravMon\) morphisms and puts them side by side. It can be seen as
...
which I believe \(\StaConfDaTravMon\) has enough flexibility to model in a
type-safe manner.
