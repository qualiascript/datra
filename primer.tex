...
\href{run:datra_preprint.pdf}{Datra preprint}, and as such, it will take a
...
is because, for instance, \(\code{[[A := 1; B := 5]; C := 8]}\) serves as
...
To illustrate why these rules are helpful, consider the case of a morphism
...
\(\code{[A := 7; [B := 2; C := 6]]}\) can be denoted as one of those
morphisms.
...
The so-called ``horizontal sum'' operator, \(\mathbin{+_{<}}\), takes two
\(\StaDaTraMon\) morphisms and puts them side by side. For instance,
\(\code{[B := 2; C := 6]}\) can be constructed as
\((B := 2) \mathbin{+_{<}} (C := 6)\). It can be clearly seen that one can
put two trees side by side, having the extent as the disjoint union of the
original two extents, so that the atlas morphisms do not get ``tangled up''.
This operator is similar to the map notation used in this document, so that
for instance,
\(A \mathbin{+_{<}} B \mathbin{+_{<}} C = \code{[A; B; C]}\). A variation
on this operation, which goes through an intermediary phase where the two
extents are kept separate on page \(1\), can be used to create maps with
complex pagination structures, such as \(\code{[A; [B; C]]}\). An
alternative symbol for \(\mathbin{+_{<}}\), which will likely be used for
the Datra programming language but was avoided in the preprint for clarity's
sake, is simply \(\code{,}\), as a concatenation operator. As such,
\(\code{1, 2, 3}\) is a simple Datra list.

\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[node distance=12mm]
  % The two operand morphisms select an element from the full set.
  \node[selected node] (bsource) {\(\{2\}\)};
  \node[atlas node,below=of bsource,minimum width=31mm] (btarget)
    {\(B=\{2,3,5\}\)};
  \draw[morphism arrow] (bsource)--node[right,font=\scriptsize]
    {\(2\mapsto2\)} (btarget);
  \node[atlas label,above=3mm of bsource] {\(B := 2\)};

  \node[font=\Large,right=12mm of bsource] (plus) {\(\mathbin{+_{<}}\)};

  \node[selected node,right=12mm of plus] (csource) {\(\{6\}\)};
  \node[atlas node,below=of csource,minimum width=31mm] (ctarget)
    {\(C=\{6,8,9\}\)};
  \draw[morphism arrow] (csource)--node[right,font=\scriptsize]
    {\(6\mapsto6\)} (ctarget);
  \node[atlas label,above=3mm of csource] {\(C := 6\)};

  \node[font=\Large,right=12mm of csource] (equals) {\(=\)};

  % Their horizontal sum retains both selections and both codomains.
  \node[selected node,right=13mm of equals] (rsourceb) {\(\{2\}\)};
  \node[selected node,right=16mm of rsourceb] (rsourcec) {\(\{6\}\)};
  \node[atlas node,below=of rsourceb,minimum width=31mm] (rtargetb)
    {\(B=\{2,3,5\}\)};
  \node[atlas node,below=of rsourcec,minimum width=31mm] (rtargetc)
    {\(C=\{6,8,9\}\)};
  \draw[morphism arrow] (rsourceb)--node[right,font=\scriptsize]
    {\(2\mapsto2\)} (rtargetb);
  \draw[morphism arrow] (rsourcec)--node[right,font=\scriptsize]
    {\(6\mapsto6\)} (rtargetc);
  \node[atlas label,above=3mm of $(rsourceb)!0.5!(rsourcec)$]
    {\(\code{[B := 2; C := 6]}\)};
  \node[draw,dashed,rounded corners=3pt,fit=(rsourceb)(rsourcec)(rtargetb)(rtargetc),
    inner sep=4mm] {};
\end{tikzpicture}%
}
\caption{Each operand morphism selects an element of its full set. Their
horizontal sum places the selections \(\{2\}\hookrightarrow B\) and
\(\{6\}\hookrightarrow C\) side by side.}
