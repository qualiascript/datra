...
data NaturalRange rangeScope federationScope = NaturalRange
  ...
  , naturalRangeInterval :: NaturalInterval
  }

naturalRangeStart :: NaturalRange rangeScope federationScope -> Natural
naturalRangeStart = naturalIntervalStart . naturalRangeInterval

naturalRangeTarget
  :: NaturalRange rangeScope federationScope
  -> NaturalRangeTarget
naturalRangeTarget = naturalIntervalTarget . naturalRangeInterval

naturalRangeDirection
  :: NaturalRange rangeScope federationScope
  -> NaturalRangeDirection
naturalRangeDirection = naturalIntervalDirection . naturalRangeInterval
    ...
    naturalRangeFederationFor valueRange start target $ \federation ->
      useRange
        NaturalRange
          ...
          , naturalRangeInterval = naturalInterval start target
          }
  where
...
naturalRangeOverlapWitness left right =
  naturalIntervalOverlapWitness
    (naturalRangeInterval left)
    (naturalRangeInterval right)
...
inclusiveEllipsisRange start target =
  superEllipsisRange
    ellipsisRank
    (finiteOrdinal start)
    (naturalRangeTargetAsSuperEllipsisRangeTarget start target)
  where
...
finiteSubrangeFits lower UpwardsTarget start target =
  directedSubintervalFits
    (naturalInterval lower UpwardsTarget) start target
finiteSubrangeFits outerStart (FiniteNaturalTarget outerTarget) start target
  = directedSubintervalFits
      (naturalInterval outerStart (FiniteNaturalTarget outerTarget))
      start
      target
